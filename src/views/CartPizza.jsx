 ...
 const { carro, agregarCarro, removerCarro, suma } = useCart()
  ...
  return (
    <div className="container mt-4">
      ...
      {carro.length ? (<section>{carro.map(({ id, img, name, price, count }) => (
            <div key={id} className="d-flex align-items-center justify-content-between mb-3 border-bottom pb-2">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                <button className="btn btn-outline-danger btn-sm"  onClick={() => removerCarro(id)}>
                  -
                </button>

                <span className="fw-bold">{count}</span>

                <button className="btn btn-outline-primary btn-sm"  onClick={() => agregarCarro({ id, img, name, price })}>
                  +
                </button>
              </div>
            </div> ))}
